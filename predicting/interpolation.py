...
spl = splrep(node[0,:],point[70,:])
x2 = np.linspace(node[0,0], node[0,4], 200)
y2 = splev(x2, spl)
plt.figure(1)
plt.plot(node[0,:],point[70,:], 'o', x2, y2)
plt.axis([0,20.1,0,1.1])

#using interp1d and barycentric interpolator
from scipy import interpolate
f = interpolate.interp1d(node[0,:], point[70,:])
f1 = interpolate.BarycentricInterpolator(node[0,:], point[70,:])
xnew = np.arange(node[0,0], node[0,4], 0.1)
ynew = f(xnew)
plt.axis([0,20.1,0,1.1])
plt.plot(node[0,:],point[70,:], 'o', xnew, ynew, '-')

#pchip interpolation
from scipy.interpolate import pchip_interpolate
x = np.linspace(node[0,0], node[0,4], num=100)
y = pchip_interpolate(node[0,:], point[70,:], x)
plt.plot(node[0,:], point[70,:], "o", label="observation")
